...
def show_exam_result(request, course_id, submission_id):
    ...
    correct_choices = course.questions.filter(choices__id__in=selected_ids, choices__is_correct=True)
    
    # Calculate the total score by adding up the grades for all questions in the course
    total_score = sum(question.grade for question in correct_choices)
    
    # Add the course, selected IDs, and grade to the context for rendering the HTML page
    context = {
        'course': course,
